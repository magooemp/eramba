...
else
{
    ...
    if(count($nodes_to_prune) > 0)
    {
        ...
        echo '<p>';
        echo $this->Html->link('<i class="icon-remove-sign icon-red icon-2x"></i> ' . __d('acl', 'Prune'), '/admin/acl/acos/prune_acos/run', array('escape' => false));
        echo '</p>';
    }
    else
    {
        echo '<p style="font-style:italic;">';
        echo '<i class="icon-ok icon-green icon-2x"></i> ' . __d('acl', 'There is no ACO node to delete');
        echo '</p>';
    }
}
