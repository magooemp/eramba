...
else
{
    ...
    if(count($missing_aco_nodes) > 0)
    {
        ...
        echo '<p>';
        echo $this->Html->link('<i class="icon-plus icon-green icon-2x"></i> ' . __d('acl', 'Build'), '/admin/acl/acos/build_acl/run', array('escape' => false));
        echo '</p>';
    }
    else
    {
        echo '<p style="font-style:italic;">';
        echo '<i class="icon-ok icon-green icon-2x"></i> ' . __d('acl', 'There is no ACO node to create');
        echo '</p>';
    }
}
