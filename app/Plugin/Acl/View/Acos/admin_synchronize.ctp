...
else
{
    ...
    if($has_aco_to_sync)
    {
        ...
        echo '<p>';
        echo $this->Html->link('<i class="icon-refresh icon-green icon-2x"></i> ' . __d('acl', 'Synchronize'), '/admin/acl/acos/synchronize/run', array('escape' => false));
        echo '</p>';
    }
    else
    {
        echo '<p style="font-style:italic;">';
        echo '<i class="icon-ok icon-green icon-2x"></i> ' . __d('acl', 'The ACO datatable is already synchronized');
        echo '</p>';
    }
}
